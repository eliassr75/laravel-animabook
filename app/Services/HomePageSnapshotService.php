...
{
    public const SNAPSHOT_KEY = 'home:landing:v1';

    private const REFRESH_AFTER_SECONDS = 600;

    public function resolve(): array
    {
        $snapshot = MetricSnapshot::query()
            ->where('metric_key', self::SNAPSHOT_KEY)
            ->first();

        if ($snapshot instanceof MetricSnapshot) {
            if ($snapshot->refresh_after?->isPast() ?? true) {
                $this->queueRefresh();
            }
    ...
    public function refresh(): array
    {
        ...
        MetricSnapshot::query()->updateOrCreate(
            ['metric_key' => self::SNAPSHOT_KEY],
            [
                'payload' => $payload,
                'generated_at' => $now,
                'refresh_after' => $now->addSeconds(self::REFRESH_AFTER_SECONDS),
            ],
        );
    ...
    {
        RefreshHomePageSnapshotJob::dispatch();
    }
    ...
    private function recentReviews(): array
    {
        ...
        $reviewIdsByType = $userReviewsRaw
            ->groupBy('media_type')
            ->map(fn (Collection $rows) => $rows->pluck('mal_id')->map(fn ($id) => (int) $id)->unique()->values());
    ...
    private function recentNews(): array
    {
        ...
        $newsIdsByType = $rows
            ->groupBy('from_type')
            ->map(fn (Collection $items) => $items->pluck('from_mal_id')->map(fn ($id) => (int) $id)->unique()->values());
    ...
    private function normalizePayload(mixed $payload): array
    {
        ...
        return $payload + $this->emptyPayload();
    }
