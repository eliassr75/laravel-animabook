...
class BackfillMangaCatalogJob implements ShouldQueue
{
    use Dispatchable;
    ...
    public function __construct(
        public readonly string $cursorName = 'manga_backfill',
        public readonly int $batchSize = 50,
    ) {
        $this->onQueue((string) config('animabook.backfill_manga.queue', 'low'));
    }
    ...
    public function handle(
        ...
        $lockTtl = max(30, (int) config('animabook.backfill_manga.lock_ttl_seconds', 180));
        $lock = Cache::lock("backfill:manga:{$this->cursorName}", $lockTtl);
        ...
        try {
            ...
            for ($malId = $nextMalId; $malId < ($nextMalId + $batchSize); $malId++) {
                if (! $budget->withinBudget('backfill_manga', 1)) {
                    $budgetExhausted = true;
                    break;
                }

                $processed++;
                $payload = $mangaEndpoint->getById($malId);
                ...
                $indexable = $mapper->map('manga', $payload);
            ...
            if ($cursor) {
                ...
                if ($cursor->is_active) {
                    $retryDelay = max(30, (int) config('animabook.backfill_manga.error_retry_seconds', 120));
    ...
    {
        $maxConsecutiveMisses = (int) config('animabook.backfill_manga.max_consecutive_misses', 5000);
    ...
    {
        if ($budgetExhausted) {
            $pauseMinutes = max(1, (int) config('animabook.backfill_manga.pause_minutes_on_budget_exhausted', 60));

            return now()->addMinutes($pauseMinutes);
        }

        $delaySeconds = max(0, (int) config('animabook.backfill_manga.delay_seconds', 2));
